...
def parse_filename(fname):
    ...
    if m:
        nodes, txpb, rounds = map(int, m.groups())
        return nodes, txpb, rounds
...
def parse_file(path, protocol, fname):
    logging.info(f"Parsing file: {fname}")
    with open(path) as f:
        content = f.read()

    nodes, txpb, rounds = parse_filename(fname)
    if not nodes:
        logging.warning(f"Filename did not match pattern: {fname}")
        return ""

    avg_tps = re.search(r"Average TPS.*?:\s*([\d.]+)", content)
    avg_overhead = re.search(r"Average Communication Overhead.*?:\s*([\d.]+)", content)
    avg_cpu = re.search(r"Average CPU Utilization.*?:\s*([\d.]+)", content)
    avg_mem = re.search(r"Average Memory Utilization.*?:\s*([\d.]+)", content)
...
"""
    ...
    if resources:
        ...
        block += ",\n".join(f"    (experiment_id, 'node-{n}', {cpu}, {mem})" for n, cpu, mem in resources) + ";\n"
...
def main():
    ...
    if not os.path.exists(full_path):
        logging.error(f"Base path does not exist: {full_path}")
        return

    for fname in sorted(os.listdir(BASE_PATH)):
        if fname.endswith('.txt'):
            lower_name = fname.lower()
            protocol = "MERKLE" if "merkle" in lower_name else "RSA" if "rsa" in lower_name else "UNKNOWN"
            path = os.path.join(BASE_PATH, fname)
            sql_block = parse_file(path, protocol, fname)
            if sql_block:
    ...
    logging.info(f"✅ SQL written to {OUTPUT_FILE}")
